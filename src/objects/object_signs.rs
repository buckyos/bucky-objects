...
pub struct ObjectSigns {
    ...
    // 表示对Desc内容的认可。
    desc_signs: Option<Vec<ObjSignature>>,

    // 对MutBody部分的签名，可以是多个。依赖MutBody的稳定编码
    body_signs: Option<Vec<ObjSignature>>,
}

pub struct ObjectSignsHelper;

impl ObjectSignsHelper {
    pub fn set_sign(list: &mut Option<Vec<ObjSignature>>, sign: ObjSignature) {
        if list.is_none() {
            *list = Some(vec![]);
        }
    ...
    pub fn push_sign(list: &mut Option<Vec<ObjSignature>>, sign: ObjSignature) {
        if list.is_none() {
            *list = Some(vec![]);
        }
    ...
    pub fn latest_sign_time(list: &Option<Vec<ObjSignature>>) -> u64 {
        let mut ret = 0;
    ...
    pub fn merge(dest: &mut Option<Vec<ObjSignature>>, src: &Option<Vec<ObjSignature>>) -> usize {
        match src {
            Some(src) => {
                match dest {
                    Some(dest) => {
                    ...
                    None => {
                        // src里面的签名也需要确保去重
                        let mut ret = 0;
                        let mut dest_list: Vec<ObjSignature> = Vec::new();
                        for src_item in src {
...
impl ObjectSignsBuilder {
    ...
    pub fn set_desc_sign(mut self, sign: ObjSignature) -> Self {
        ObjectSignsHelper::set_sign(&mut self.desc_signs, sign);
        self
    }

    pub fn set_body_sign(mut self, sign: ObjSignature) -> Self {
        ObjectSignsHelper::set_sign(&mut self.body_signs, sign);
        self
    }

    pub fn push_desc_sign(mut self, sign: ObjSignature) -> Self {
        ObjectSignsHelper::push_sign(&mut self.desc_signs, sign);
        self
    }

    pub fn push_body_sign(mut self, sign: ObjSignature) -> Self {
...
impl ObjectSigns {
    ...
    pub fn desc_signs(&self) -> Option<&Vec<ObjSignature>> {
        self.desc_signs.as_ref()
    }

    pub fn body_signs(&self) -> Option<&Vec<ObjSignature>> {
        self.body_signs.as_ref()
    }

    pub fn set_desc_sign(&mut self, sign: ObjSignature) {
        ObjectSignsHelper::set_sign(&mut self.desc_signs, sign)
    }

    pub fn set_body_sign(&mut self, sign: ObjSignature) {
        ObjectSignsHelper::set_sign(&mut self.body_signs, sign)
    }

    pub fn push_desc_sign(&mut self, sign: ObjSignature) {
        ObjectSignsHelper::push_sign(&mut self.desc_signs, sign)
    }

    pub fn push_body_sign(&mut self, sign: ObjSignature) {
        ObjectSignsHelper::push_sign(&mut self.body_signs, sign)
    }
...
impl<'de> RawDecodeWithContext<'de, &NamedObjectContext> for ObjectSigns {
    fn raw_decode_with_context(
        ...
        let (desc_signs, buf) = if ctx.has_desc_signs() {
            let (desc_signs, buf) = Vec::<ObjSignature>::raw_decode(buf).map_err(|e| {
                log::error!(
        ...
        let (body_signs, buf) = if ctx.has_body_signs() {
            let (body_signs, buf) = Vec::<ObjSignature>::raw_decode(buf).map_err(|e| {
                log::error!(
